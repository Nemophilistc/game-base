...
# Use raw string to keep \u as literal characters in the HTML
CORRECT_WARNING = r'''<script>
...
for d in sorted(os.listdir(BASE)):
    ...
    # Remove any existing file:// warning (garbled or correct)
    content = re.sub(r'<script>\s*if\(location\.protocol==="file:"\)[\s\S]*?</script>\s*', '', content)

    # Add correct warning before </body>
    if '</body>' in content:
        content = content.replace('</body>', CORRECT_WARNING + '\n</body>')
